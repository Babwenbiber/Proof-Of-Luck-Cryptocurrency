...
typedef struct ms_ecall_get_proof_t {
	sgx_status_t ms_retval;
	double* ms_luckyNumber;
} ms_ecall_get_proof_t;

typedef struct ms_ecall_get_counterKey_t {
	sgx_status_t ms_retval;
	uint8_t* ms_keyX;
	uint8_t* ms_keyY;
} ms_ecall_get_counterKey_t;

typedef struct ms_ocall_print_string_t {
	char* ms_str;
} ms_ocall_print_string_t;

typedef struct ms_ocall_print_uint8_t_t {
	uint8_t ms_num;
} ms_ocall_print_uint8_t_t;

typedef struct ms_ocall_sleep_t {
	unsigned int ms_time;
} ms_ocall_sleep_t;

typedef struct ms_ocall_save_in_file_t {
	int ms_retval;
	char* ms_fileName;
	uint8_t* ms_data;
	uint32_t ms_sealedDataSize;
} ms_ocall_save_in_file_t;
...
static sgx_status_t SGX_CDECL Enclave_ocall_print_uint8_t(void* pms)
{
	ms_ocall_print_uint8_t_t* ms = SGX_CAST(ms_ocall_print_uint8_t_t*, pms);
	ocall_print_uint8_t(ms->ms_num);
...
static sgx_status_t SGX_CDECL Enclave_ocall_save_in_file(void* pms)
{
	ms_ocall_save_in_file_t* ms = SGX_CAST(ms_ocall_save_in_file_t*, pms);
	ms->ms_retval = ocall_save_in_file((const char*)ms->ms_fileName, ms->ms_data, ms->ms_sealedDataSize);
...
static const struct {
	...
	13,
	{
		...
		(void*)Enclave_ocall_print_uint8_t,
		(void*)Enclave_ocall_sleep,
		(void*)Enclave_ocall_save_in_file,
...
sgx_status_t ecall_get_proof(sgx_enclave_id_t eid, sgx_status_t* retval, double* luckyNumber)
{
	sgx_status_t status;
	ms_ecall_get_proof_t ms;
	ms.ms_luckyNumber = luckyNumber;
	status = sgx_ecall(eid, 0, &ocall_table_Enclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}

sgx_status_t ecall_get_counterKey(sgx_enclave_id_t eid, sgx_status_t* retval, uint8_t* keyX, uint8_t* keyY)
{
	sgx_status_t status;
	ms_ecall_get_counterKey_t ms;
	ms.ms_keyX = keyX;
	ms.ms_keyY = keyY;
